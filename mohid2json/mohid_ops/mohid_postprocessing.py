...
###############################################################################
        ...
        plt.text(-44.77,-22.92,f'{np.int64(time[2])}/{np.int64(time[1])}/{np.int64(time[0])} {np.int64(time[3])}:00', color='w')
...
def json_files(BASE_DIR,today_date):
...
# Geographic coordinates
###############################################################################
    lon = f[('Grid/Longitude')][0:-1,0:-1]
    lat = f[('Grid/Latitude')][0:-1,0:-1]

    #print(lon.T.flatten()[::-1])
    #print(lat.T.flatten()[::-1])
    ...
    dx = abs(lon[30][0]-lon[31][0])
    dy = abs(lat[0][30]-lat[0][31])
    nx = np.size(lon, 0)
    ny = np.size(lat, 1)
...
# Define Time List
###############################################################################
    idt = [f'000{i}' for i in range(13, 37)]
...
###############################################################################
    for k, val in enumerate(idt):
        time = f['Time/Time_'+val][:]
        refTime = f'{int(time[0])}-{str(int(time[1])).zfill(2)}-{str(int(time[2])).zfill(2)} {str(int(time[3])).zfill(2)}:00:00'
        u = f[('Results/velocity U/velocity U_'+val)][0,:,:].T
        u = u[::-1,:]
        #u[u == 0] = np.nan 
        v = f[('Results/velocity V/velocity V_'+val)][0,:,:].T
        v = v[::-1,:]
        #v[v == 0] = np.nan
        ...
        jsonFile = [{"header": {"parameterUnit": "m.s-1",
                        "parameterNumber": 2,
                        "dx": dx, 
                        "dy": dy,
        ...
        # convert into JSON: f'data_{k}.json'
        with open(f'big_{k}.json', 'w') as file:
            json.dump(jsonFile, file)
            print(f'JSON {k} created!')
    

def update_website(BASE_DIR, pass_file):
    """Read the FTP credentials from external file"""
    with open(os.path.join(BASE_DIR, pass_file), 'r') as file:
        log = file.read()
        host = log.split('\n')[0]
        username = log.split('\n')[1]
        passw = log.split('\n')[2]        

    img_dir = os.path.join(BASE_DIR, 'figures/')       
    files = os.listdir(img_dir)

    ftp = ftplib.FTP(host, username, passw)
    ftp.encoding = "utf-8"
